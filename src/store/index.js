...
export default new Vuex.Store({
  ...
  getters: {
    cursoTerminado: (state) => (id) => {    
      let curso = state.cursos.find(curso => curso.id === id) 
    ...
    totalAlumnosPermitidos: state => {
      const alumnosPermitidos = state.cursos.reduce((acumulador, actual) => acumulador + Number(actual.cupos), 0)
      return  alumnosPermitidos
    },
    totalAlumnosInscritos: state => {
      const alumnosInscritos = state.cursos.reduce((acumulador, actual) => acumulador + Number(actual.inscritos), 0)
    ...
    totalCursosTerminados:state =>{
      const cursosTerminados = state.cursos.reduce((acumulador, actual) => acumulador + actual.completado, 0)
      return  cursosTerminados
    },
    totalCursosActivos: (state, getters) => {
      return getters.totalCursos - getters.totalCursosTerminados
    },
    totalCursos: state => {
      return state.cursos.length
    },
    obtenerCurso:(state) => (id) => {
      let cursoEncontrado = state.cursos.find(curso => curso.id == id)
      return cursoEncontrado
    }
  },
  mutations: {
    AGREGAR_CURSO: (state, curso) =>{
      state.cursos.push(curso)
    },
    ELIMINAR_CURSO: (state, cursoId) =>{
      let index = state.cursos.findIndex(curso => curso.id == cursoId)
  ...
  actions: {
    agregar_curso: ({commit},curso) =>{
      commit('AGREGAR_CURSO', curso)
    },
    eliminar_curso:({commit}, cursoId)=>{
      commit('ELIMINAR_CURSO', cursoId)
    },
    editar_curso:({commit}, curso)=>{
       commit('EDITAR_CURSO', curso)
     },
  },
  modules: {
  }
